    ...
    def handle(self, *args, **options):
        ...
        if Tournament.objects.count()==0:
            Tournament.objects.create(
                title='Demo Cup A', game_name='FC Mobile', start_datetime=timezone.now()+datetime.timedelta(days=2),
                entry_fee=5.00, prize_first='500', prize_second='250', prize_third='100', created_by=User.objects.first()
            )
            Tournament.objects.create(
                title='Demo Cup B', game_name='Clash Royale', start_datetime=timezone.now()+datetime.timedelta(days=5),
